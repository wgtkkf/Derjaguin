# Derjaguin approximation for Electron (W/m2K to nW/K)
...
while gap < gapmax:
    ...
    while sr < srmax:
        ...
        ind_h = liner_interpolate(dtldr/ucnano)[0] # ([nm]), [W/m2K]
        ...
        ind_cd = liner_interpolate(dtldr/ucnano)[1] # ([nm]), [A/m2]

        # individual current
        ind_c = ind_cd*(2*np.pi*sr)*dsr # [A], (A/m2)*m2

        # current (summation)
        total_c = total_c + ind_c # [A]
    ...
    f0.write(str(' '))
    f0.write(str(total_c/ucnano)) # total conductance [nA]
    f0.write('\n')
